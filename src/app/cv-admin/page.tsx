...
export default async function AdminPage() {
  ...
  const data = await getCvData({ fallbackToSeed: false });
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10">
        ...
        <AdminTabs tabs={tabs} initialTab="personal">
          <section className="space-y-6">
            ...
            <form action={savePersonalInfo} className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6">
              ...
              <div>
                <button className="w-full rounded-2xl bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90">
                  Save personal info
                </button>
              </div>
            </form>
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-5">
              {data.education.map((entry) => (
                <form key={entry.id} action={saveEducation} className="rounded-3xl border border-slate-200 bg-white p-5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button className="rounded-2xl bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white">Save</button>
                    <button formAction={deleteEducation} name="id" value={entry?.id ?? ""} className="rounded-2xl border border-red-200 px-4 py-2 text-sm text-red-500">
                      Delete
                    </button>
                  </div>
                </form>
              ...
              <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-5">
                ...
                <form action={saveEducation} className="mt-3 grid gap-4 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    <button className="w-full rounded-2xl border border-[var(--accent)] px-4 py-2 text-sm text-[var(--accent)]">
                      Add entry
                    </button>
                  </div>
                </form>
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-5">
              {data.skillCategories.map((category) => (
                <div key={category.id ?? category.name} className="rounded-3xl border border-slate-200 bg-white p-5">
                  <form action={saveSkillCategory} className="grid gap-3 md:grid-cols-[2fr,1fr,auto] md:items-end">
                    ...
                    <div className="flex gap-2">
                      <button className="flex-1 rounded-2xl bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white">Save</button>
                      {category.id ? (
                        <button
                          formAction={deleteSkillCategory}
                          name="id"
                          value={category.id}
                          className="rounded-2xl border border-red-200 px-4 py-2 text-sm text-red-500"
                        >
                          Delete
                        </button>
                      ) : null}
                  ...
                  <div className="mt-4 space-y-3">
                    {category.skills.map((skill) => (
                      <form key={skill.id ?? skill.name} action={saveSkill} className="grid gap-3 md:grid-cols-[2fr,1fr,1fr,auto] md:items-end">
                        ...
                        <div className="flex gap-2">
                          <button className="flex-1 rounded-2xl border border-[var(--accent)] px-4 py-2 text-sm text-[var(--accent)]">Update</button>
                          {skill.id ? (
                            <button
                  ...
                  {category.id ? (
                    <form action={saveSkill} className="mt-4 grid gap-3 md:grid-cols-[2fr,1fr,1fr,auto] md:items-end">
                      ...
                      <TextField label="Order" name="order" type="number" defaultValue="0" />
                      <button className="rounded-2xl border border-[var(--accent)] px-4 py-2 text-sm text-[var(--accent)]">Add skill</button>
                    </form>
                  ) : null}
                </div>
              ))}
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-5">
              {data.experiences.map((exp) => (
                <form key={exp.id} action={saveExperience} className="rounded-3xl border border-slate-200 bg-white p-5">
                  ...
                  <div className="mt-4 flex gap-3">
                    <button className="rounded-2xl bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white">Save experience</button>
                    <button formAction={deleteExperience} name="id" value={exp?.id ?? ""} className="rounded-2xl border border-red-200 px-4 py-2 text-sm text-red-500">
                      Delete
                    </button>
                  </div>
                </form>
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-5">
              {data.projects.map((project) => (
                <div key={project.id ?? project.slug} className="space-y-4 rounded-3xl border border-slate-200 bg-white p-5">
                  <form action={saveProject} className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="md:col-span-2 flex flex-wrap gap-3">
                      <button className="rounded-2xl bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white">Save project</button>
                      <button formAction={deleteProject} name="id" value={project.id ?? ""} className="rounded-2xl border border-red-200 px-4 py-2 text-sm text-red-500">
                        Delete
                      </button>
                    </div>
                  </form>
                    ...
                    <div className="space-y-3">
                      ...
                      {project.links.map((link) => (
                        <form key={link.id ?? link.label} action={saveProjectLink} className="grid gap-2 rounded-2xl border border-slate-200 p-3">
                          ...
                          <div className="flex gap-2">
                            <button className="flex-1 rounded-2xl border border-[var(--accent)] px-3 py-2 text-xs text-[var(--accent)]">Update</button>
                            <button formAction={deleteProjectLink} name="id" value={link.id ?? ""} className="rounded-2xl border border-red-200 px-3 py-2 text-xs text-red-500">
                              Remove
                            </button>
                          </div>
                        </form>
                      ...
                      {project.id ? (
                        <form action={saveProjectLink} className="grid gap-2 rounded-2xl border border-dashed border-slate-300 p-3">
                          ...
                          <TextField label="Order" name="order" type="number" defaultValue="0" />
                          <button className="rounded-2xl border border-[var(--accent)] px-3 py-2 text-xs text-[var(--accent)]">
                            Add link
                          </button>
                        </form>
                      ) : null}
                    ...
                    <div className="space-y-3">
                      ...
                      {project.images.map((image) => (
                        <form key={image.id ?? image.imageUrl} action={saveProjectImage} className="grid gap-2 rounded-2xl border border-slate-200 p-3">
                          ...
                          <div className="flex gap-2">
                            <button className="flex-1 rounded-2xl border border-[var(--accent)] px-3 py-2 text-xs text-[var(--accent)]">Update</button>
                            <button formAction={deleteProjectImage} name="id" value={image.id ?? ""} className="rounded-2xl border border-red-200 px-3 py-2 text-xs text-red-500">
                              Remove
                            </button>
                          </div>
                        </form>
              ...
              <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-5">
                ...
                <form action={saveProject} className="mt-3 grid gap-4 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    <button className="w-full rounded-2xl border border-[var(--accent)] px-4 py-2 text-sm text-[var(--accent)]">Add project</button>
                  </div>
                </form>
              </div>
            </div>
